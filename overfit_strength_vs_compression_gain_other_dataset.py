...
from torchvision.datasets import Flowers102, StanfordCars, SUN397
...
# Build datasets
# ----------------------------
def build_dataset_base(name: str, root: str):
    """
    Returns: base_raw (ConcatDataset if multiple splits), class_count
    """
    name = name.lower()

    if name == "flowers102":
        # splits: train/val/test
        ds_train = Flowers102(root=root, split="train", download=True, transform=None)
        ds_val   = Flowers102(root=root, split="val",   download=True, transform=None)
        ds_test  = Flowers102(root=root, split="test",  download=True, transform=None)
        base_raw = ConcatDataset([ds_train, ds_val, ds_test])
        num_classes = 102  # fixed
        return base_raw, num_classes

    if name == "stanfordcars":
        # splits: train/test
        ds_train = StanfordCars(root=root, split="train", download=True, transform=None)
        ds_test  = StanfordCars(root=root, split="test",  download=True, transform=None)
        base_raw = ConcatDataset([ds_train, ds_test])
        num_classes = 196  # fixed
        return base_raw, num_classes
    
    if name == "sun397":
        # SUN397 has no train/test split in torchvision; it's a single dataset.
        # We'll rely on our region + 50/50 split logic anyway.
        ds = SUN397(root=root, download=True, transform=None)
        base_raw = ds
        num_classes = 397
        return base_raw, num_classes

    raise ValueError("dataset must be one of: flowers102, stanfordcars, sun397")
...
def pick_region_and_split(
    ...
    """
    Contiguous region [offset, offset+region_size), then RANDOM 50/50 split inside region.
    """
...
def get_regimes() -> List[Regime]:
    return [
        Regime("strong_reg",  pretrained=True,  epochs=6,  lr=1e-4, weight_decay=3e-3, label_smoothing=0.15, aug=True),
        Regime("medium_reg",  pretrained=True,  epochs=10, lr=2e-4, weight_decay=1e-3, label_smoothing=0.10, aug=True),
        Regime("weak_reg",    pretrained=True,  epochs=14, lr=3e-4, weight_decay=3e-4, label_smoothing=0.05, aug=True),
        Regime("overfit_hard",pretrained=False, epochs=2, lr=1e-3, weight_decay=0.0, label_smoothing=0.00, aug=False),
    ]
...
def main():
    ap = argparse.ArgumentParser(
        description="Same overfit-vs-compression experiment, but on Flowers102 or StanfordCars."
    )
    ap.add_argument("--dataset", type=str, default="flowers102", choices=["flowers102", "stanfordcars" , "sun397"])
    ...
    root = os.getcwd()
    outdir = "compressed_out"
    os.makedirs(outdir, exist_ok=True)

    base_raw, num_classes = build_dataset_base(args.dataset, root)

    # Fixed region + fixed split
    ...
    # Build ONE training tar for the chunk
    train_tar_u = os.path.join(outdir, f"{args.dataset}_train_chunk_uncompressed.tar")
    make_train_tar(train_tar_u, raw_region, train_local, args.compress_limit)

    # Compress training tar once
    train_zst = os.path.join(outdir, f"{args.dataset}_train_chunk.tar.zst")
    train_lz  = os.path.join(outdir, f"{args.dataset}_train_chunk.tar.lzma")
    ...
    for r in regimes:
        ...
        ckpt_path = os.path.join(outdir, f"{args.dataset}_model_{r.name}.pth")
        torch.save({"state": best_state, "dataset": args.dataset, "regime": r.__dict__}, ckpt_path)

        model_tar_u = os.path.join(outdir, f"_{args.dataset}_model_{r.name}.tar")
        both_tar_u  = os.path.join(outdir, f"_{args.dataset}_both_{r.name}.tar")
        make_model_tar(model_tar_u, ckpt_path, train_tar_path=None)
        make_model_tar(both_tar_u,  ckpt_path, train_tar_path=train_tar_u)

        model_zst = os.path.join(outdir, f"{args.dataset}_model_{r.name}.tar.zst")
        both_zst  = os.path.join(outdir, f"{args.dataset}_both_{r.name}.tar.zst")
        model_lz  = os.path.join(outdir, f"{args.dataset}_model_{r.name}.tar.lzma")
        both_lz   = os.path.join(outdir, f"{args.dataset}_both_{r.name}.tar.lzma")
        ...
        gain_zst     = (train_zst_sz + model_zst_sz) - both_zst_sz

        model_lz_sz = fsize(model_lz)
        both_lz_sz  = fsize(both_lz)
        gain_lz     = (train_lz_sz + model_lz_sz) - both_lz_sz

        print(f"Compression (Zstd): gain={sizeof_fmt(gain_zst)}")
        print(f"Compression (LZMA): gain={sizeof_fmt(gain_lz)}")
        print(f"Last-epoch stats: train_acc={last_train_acc:.4f}, val_acc={last_val_acc:.4f}, gap={last_gap:.4f}")
    ...
    out_json = os.path.join(outdir, f"results_{args.dataset}_overfit_vs_gain.json")
